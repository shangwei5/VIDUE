...
class VIDEODATA(data.Dataset):
    def __init__(self, args, name='', train=True):
        ...
        if self.random:
            self.exposure = [i+1 for i in range(self.xN)]
            self.readout = [self.xN-j for j in self.exposure]
            if self.xN > 8:
                self.exposure = [i + 1 for i in range(0,self.xN,2)]
                self.readout = [self.xN - j for j in self.exposure]
    ...
    def _load_file(self, idx):
        ...
        f_inputs = self.images[video_idx][frame_idx:frame_idx + self.n_seq:self.xN]
        inputs = []
        frame = imageio.imread(f_inputs[0])
        frame = cv2.resize(frame, None, fx=0.5, fy=0.5)
        img = np.array(frame)
        H, W, C = img.shape
        for i in range(frame_idx,frame_idx + self.n_seq,self.xN):
            blur = np.zeros((H, W, C))
