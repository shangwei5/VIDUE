...
class VIDEODATA(data.Dataset):
    def __init__(self, args, name='', train=True):
        ...
        if self.random:
            self.exposure = [(i+1) * self.args.blur_deg for i in range(self.xN//2, self.xN, 2)]
            self.readout = [(self.xN * self.args.blur_deg - j) for j in self.exposure]
            self.curr_exposure = self.exposure[0] * self.args.blur_deg
        else:
    ...
    def _load_file(self, idx):
        ...
        if self.in_seq % 2 == 0:
            f_gts = self.images[video_idx][frame_idx + self.xN * (self.in_seq // 2-1) * self.args.blur_deg+self.args.blur_deg//2:frame_idx + self.xN * (self.in_seq // 2) * self.args.blur_deg:self.args.blur_deg]
        else:
            f_gts = self.images[video_idx][frame_idx + self.xN * (
                        self.in_seq // 2) * self.args.blur_deg + self.args.blur_deg // 2:frame_idx + self.xN * (
                        self.in_seq // 2+1) * self.args.blur_deg:self.args.blur_deg]
        f_inputs = self.images[video_idx][frame_idx+self.curr_exposure//2:frame_idx + self.n_seq * self.args.blur_deg:self.xN * self.args.blur_deg]
        inputs = []
        frame_gt = imageio.imread(f_gts[0])
        frame_gt = cv2.resize(frame_gt, None, fx=0.5, fy=0.5)
        gt_img = np.array(frame_gt)
        H, W, C = gt_img.shape
        for i in range(frame_idx+self.curr_exposure//2,frame_idx + self.n_seq * self.args.blur_deg,self.xN * self.args.blur_deg):
            blur = np.zeros((H, W, C))
            for j in range(i-self.curr_exposure//2,i+self.curr_exposure//2+1):
                # print(self.images[video_idx][j])
    ...
    def _load_file_from_loaded_data(self, idx):
        ...
        if self.in_seq % 2 == 0:
            f_gts = self.images[video_idx][frame_idx + self.xN * (
                        self.in_seq // 2 - 1) * self.args.blur_deg + self.args.blur_deg // 2:frame_idx + self.xN * (
                        self.in_seq // 2) * self.args.blur_deg:self.args.blur_deg]
        else:
            f_gts = self.images[video_idx][frame_idx + self.xN * (
                    self.in_seq // 2) * self.args.blur_deg + self.args.blur_deg // 2:frame_idx + self.xN * (
                    self.in_seq // 2 + 1) * self.args.blur_deg:self.args.blur_deg]
        ...
        if self.in_seq % 2 == 0:
            gts = self.data_images[video_idx][
                    frame_idx + self.xN * (self.in_seq // 2 - 1) * self.args.blur_deg+ self.args.blur_deg // 2:frame_idx + self.xN * (self.in_seq // 2) * self.args.blur_deg:self.args.blur_deg]
            # print("gt:", [ind for ind in range(frame_idx + self.xN * (self.in_seq // 2 - 1) * self.args.blur_deg+ self.args.blur_deg // 2,frame_idx + self.xN * (self.in_seq // 2) * self.args.blur_deg,self.args.blur_deg)])
        else:
            gts = self.data_images[video_idx][
                    frame_idx + self.xN * (self.in_seq // 2) * self.args.blur_deg + self.args.blur_deg // 2:frame_idx + self.xN * (
                            self.in_seq // 2 + 1) * self.args.blur_deg:self.args.blur_deg]
        gts = np.array(gts)
        inputs = np.array(inputs)
        output_filenames = [os.path.split(os.path.dirname(name))[-1] + '.' + os.path.splitext(os.path.basename(name))[0]
    ...
    def _load_file_test(self, idx):
        ...
        if self.in_seq % 2 == 0:
            ...
            f_gts = self.images[video_idx][frame_idx + self.xN * (
                    self.in_seq // 2) * self.args.blur_deg + self.args.blur_deg // 2:frame_idx + self.xN * (
                    self.in_seq // 2 + 1) * self.args.blur_deg:self.args.blur_deg]
        f_inputs = self.images[video_idx][frame_idx+exposure//2:frame_idx + self.n_seq * self.args.blur_deg:self.xN * self.args.blur_deg]
        inputs = []
        frame_gt = imageio.imread(f_gts[0])
        frame_gt = cv2.resize(frame_gt, None, fx=0.5, fy=0.5)
        gt_img = np.array(frame_gt)
        H, W, C = gt_img.shape
        for i in range(frame_idx+exposure//2,frame_idx + self.n_seq * self.args.blur_deg,self.xN * self.args.blur_deg):
            blur = np.zeros((H, W, C))
