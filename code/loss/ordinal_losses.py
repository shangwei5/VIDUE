...
class OrdinalSupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        if labels is not None and mask is not None:
            ...
            labels_r = labels.repeat(2,1)
            weights = torch.abs((labels_r-labels_r.T)).float().to(device)
        else:
            mask = mask.float().to(device)

        contrast_count = features.shape[1]  #2
        contrast_feature = torch.cat(torch.unbind(features, dim=1), dim=0)   #2b, c
        ...
        exp_logits_sum = torch.matmul(exp_logits, weights)
