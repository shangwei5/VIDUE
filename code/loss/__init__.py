...
# from loss.coral_losses import CoralLoss, Niuloss
...
class Loss(nn.modules.loss._Loss):
    def __init__(self, args, ckp):
        ...
        device = torch.device('cpu' if args.cpu else 'cuda'+':'+str(args.n_GPUs-1))
        ...
        for loss in args.loss.split('+'):
            ...
            if loss_type == 'MSE':
                ...
                loss_function = SupConLoss(device=device)
            elif loss_type == 'OrdinalSupCon':
                loss_function = OrdinalSupConLoss(device=device)
            elif loss_type == 'Coral':
                loss_function = CoralLoss()
            elif loss_type == 'Niuloss':
                loss_function = Niuloss()
